...
*/
	public function GrabSingleFrame($pos)
	{
		...
		return array('filename' => $pos.'.jpg',
				'url' => Path::Get('rel:admin/temp').'/'.$this->dir.'/'.$pos.'.jpg');
...
//		echo "STEP $step<br>";
		...
		if($this->mplayer)
		{
			...
			while($pos <= $this->cache['length'])
			{
				...
				if(self::$debug) Videoprocessor::debug_msg( $thgrab['filename'], false ); //no br
...
//				echo ($pos / $this->cache['length']) * 100;
				VideoProcessor::$progressBar->update($pos / $this->cache['length'] * 100, $data);
