...
class flickr_plugin extends SocialPlugin {
	...
	//from 1-7, although 7 is not used as of writing this
	...
	{
		//we need something to post to flickr
		...
		if(isset($this->options['access_token']['auth']['token']['_content']))
			$this->access_token = $this->options['access_token']['auth']['token']['_content'];
		else
		...
		if($this->access_token)
		{
			...
			foreach($update['freeimages'] as $photo)
			{
				...
				//add tags to images
				$tags = (self::ADD_TAGS && !empty($update['tags']) ? implode(" ", $update['tags']) : null);

				//safety level; deactivated by default
				$safety_level = null; //set this to 1..3 according to your site

				if(SocialPosting::$debug) Tools::debug_msg( "Uploading $photo[filename]...", false ); //no br

					//upload
					$id = $this->Upload($photo['path'], $update['title'], $description, $tags, $is_public, $safety_level, $content_type, $hidden);


				if($id)
				{
					if(SocialPosting::$debug) Tools::debug_msg( "ok", (self::LICENSE_ID ? false : true) ); //br switch
					...
					if(self::LICENSE_ID)
					{
						if(SocialPosting::$debug) Tools::debug_msg( " | setting license...", false ); //no br
						...
						if(SocialPosting::$debug) Tools::debug_msg( " ok.", false ); //br added by progressbar
					}

				}
				else //no id came back from flickr
					if(SocialPosting::$debug) Tools::debug_msg( " fail.", false ); //br added by progressbar
			...
			//create photoset (if more than one free image selected)
...
//die();
	...
	public function Authorize()
	{
		...
		if(empty(self::$configuration['key']) || empty(self::$configuration['secret']))
		{
			return false;
		}
	...
	function Upload($photo, $title = null, $description = null, $tags = null, $is_public = null, $safety_level = null, $content_type = 1, $hidden = 1, $is_friend = null, $is_family = null) {
		...
		$args = array(
			"api_key" => $this->options['key'],
			"auth_token" => ($this->access_token ? $this->access_token : null),
		...
		//photo for upload (not in signature)
		//the '@' prefix is important (to mark option as file upload)
		$args['photo'] = '@'.$photo;
		...
		foreach ($rsp as $line)
		{
			...
			{
				if(SocialPosting::$debug) Tools::debug_msg( " [error $match[1]: $match[2]].", false );
				return false;
			}
			elseif (ereg("<photoid>(.*)</", $line, $match))
			{
				if(SocialPosting::$debug) Tools::debug_msg( " [ok; photoid=$match[1]].", false );				return $match[1];
			}
		}
	}
