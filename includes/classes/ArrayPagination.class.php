...
class ArrayPagination
{
	...
	var $perPage = 12; // Items on each page, defaulted to 12
	var $showFirstAndLast = false; // if you would like the first and last page options.

    function generate($array, $perPage = 12)
    {
      ...
      if (!empty($_GET['page'])) {
        $this->page = intval($_GET['page']); // using the get method
      } else {
      ...
      $slinks = array();
		...
		if (count($_GET)) {
			foreach ($_GET as $key => $value) {
				if ($key != 'page') {
					$queryURL .= '&amp;'.$key.'='.intval($value); //we are only passing integers!!!
				}
			}
		}
