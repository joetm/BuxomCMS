...
class OutlineEngine implements IOutlineEngine {
  ...
  */
	...
	public $config = array(
		...
		"compiled_path" =>       OUTLINE_COMPILED_PATH,   /* folder containing compiled templates (must be writable) */
		...
		"cache_path" =>          OUTLINE_COMPILED_PATH,   /* The folder in which the Cache class stores it's content*/
  ...
  */
  ...
  protected $caching = true;
    ...
    */
		...
		if ((count(self::$engine_stack) == 1) && $this->config['quiet'])
			self::$error_level = error_reporting(__ERROR_REPORTING_QUIET);
		else
			self::$error_level = error_reporting(__ERROR_REPORTING_DEBUG);
  ...
  // --- Support functions for compiled templates:
    ...
    */
		...
		if ((count(self::$engine_stack) == 1) && self::$engine_stack[0]->config['quiet'])
