...
if (basename(__FILE__) == basename($_SERVER['PHP_SELF']))
  die('This script is included from one of the other scripts.');
...
//		$akismet = new Akismet('http://www.buxomcurves.com/', $AkismetAPIKey, $comment);

		// Test for errors.
		if($this->errorsExist()) { // Returns true if any errors exist.
		        if($this->isError('AKISMET_INVALID_KEY')) {
		                // Do something.

		        } elseif($this->isError('AKISMET_RESPONSE_FAILED')) {
		                // Do something.

		        } elseif($this->isError('AKISMET_SERVER_NOT_FOUND')) {
		                // Do something.
		        }
		} else {
		        // No errors, check for spam.
		        if ($this->isSpam()) { // Returns true if Akismet thinks the comment is spam.
